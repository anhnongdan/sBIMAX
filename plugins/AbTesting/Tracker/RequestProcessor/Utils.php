...
class Utils extends Tracker\RequestProcessor
{
    ...
    public function isValidTarget(Request $request, $experiment)
    {
        if (!empty($experiment['excluded_targets'])) {
            foreach ($experiment['excluded_targets'] as $excludedTarget) {
                $target = new Target($excludedTarget);
        ...
        if (!empty($experiment['included_targets'])) {
            foreach ($experiment['included_targets'] as $includedTarget) {
                $target = new Target($includedTarget);
                if ($target->matches($request)) {
                    return true;
                }
            }
        ...
        // 1 == true, easier to make sure the tests actually test behaviour of included_targets this way.
        return 1;
    }

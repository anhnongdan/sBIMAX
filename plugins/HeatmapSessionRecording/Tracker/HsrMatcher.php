...
class HsrMatcher
{
    ...
    public static function checkIsNotEnded($hsr)
    {
        ...
        if ($hsr['record_type'] == SiteHsrDao::RECORD_TYPE_HEATMAP) {
            // in worst case we execute this query like 1000, 2000, or 5000 times which shouldn't be too bad as it is also fast
            $numSamples = $logHsrSite->getNumPageViews($idSiteHsr);

            if (!empty($hsr['sample_limit']) && $numSamples >= $hsr['sample_limit']) {

...
class LogHsr
{
    ...
    public function install()
    {
        DbHelper::createTable($this->table, "
                  ...
                  `time_on_page` MEDIUMINT(8) NOT NULL,
    ...
    public function findIdLogHsr($idVisit, $idHsrView)
    {
        ...
        return $this->db->fetchOne($query, array($idVisit, $idHsrView));
    }
    ...
    public function record($hsrSiteIds, $idSite, $idVisit, $idHsrView, $idPageview, $url, $serverTime, $userAgent, $resolution, $timeOnPage, $viewportW, $viewportH, $scrollYMaxRelative, $foldYRelative)
    {
        ...
        if (empty($idLogHsr)) {
            ...
            Piwik::postEvent('HeatmapSessionRecording.trackNewHsrSiteIds', array(&$hsrSiteIds, array('idSite' => $idSite, 'serverTime' => $serverTime)));
            ...
            $sql = sprintf('INSERT INTO %s (`%s`) VALUES(?,?,?,?,?,?,?,?,?,?) 
                          ...
                          idloghsr = LAST_INSERT_ID(idloghsr)', $this->tablePrefixed, $columns);
            ...
            $all = $this->db->rowCount($result);

            $idLogHsr = $this->db->lastInsertId();
            ...
            if ($all === 1 || $all === '1') {
                ...
                $this->db->query(sprintf('UPDATE %s set idaction_url = ?, device_type = ? where idloghsr = ?', $this->tablePrefixed),

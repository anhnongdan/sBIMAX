...
class LogHsrSite
{
    ...
    public function linkRecord($idLogHsr, $idSiteHsr)
    {
        ...
        $sql = sprintf('INSERT INTO %s (`idloghsr`, `idsitehsr`) VALUES(?,?) 
                        ON DUPLICATE KEY UPDATE idloghsr = idloghsr', $this->tablePrefixed);
    ...
    public function getNumPageViews($idSiteHsr)
    {
        ...
        return (int) $this->db->fetchOne($sql, array($idSiteHsr));
    }

    // should be fast as covered index
    public function getNumSessions($idSiteHsr)
    {
        $sql = sprintf('SELECT count(distinct idvisit) FROM %s loghsrsite left join %s loghsr on loghsr.idloghsr = loghsrsite.idloghsr WHERE loghsrsite.idsitehsr = ?', $this->tablePrefixed, Common::prefixTable('log_hsr'));
    ...
    public function unlinkRecord($idLogHsr, $idSiteHsr)
    {
        ...
        return $this->db->query($sql, array($idSiteHsr, $idLogHsr));
    }
    ...
    public function unlinkSiteRecords($idSiteHsr)
    {
        ...
        return $this->db->query($sql, array($idSiteHsr));
    }

...
(function () {
    ...
    HeatmapVisController.$inject = ['$scope', 'piwikApi', 'piwik', 'piwikUrl', '$timeout', '$q', '$filter', '$sce'];
    ...
    function scrollHeatmap(recordingIframe, scrollReaches)
    {
        ...
        var buckets = [];
        var num_viewers = 0, lastBucket = null, percentage = 100, scrollReach, scrollDepth;
        for (var i = 0; i < scrollReaches.length; i++) {

            scrollReach = parseInt(scrollReaches[i].value, 10);
            scrollDepth = parseInt(scrollReaches[i].label, 10);

            var reach = Math.round(scrollDepth * heightToIntervalRatio);

            if (lastBucket && lastBucket.position === reach) {
                ...
                lastBucket = {percentageValue: percentage.toFixed(1) * 10, position: reach, percent: percentage.toFixed(1)};
    ...
    function HeatmapVisController($scope, piwikApi, piwik, piwikUrl, $timeout, $q, $filter, $sce) {
        var self = this;
        var deviceDesktop = 1;
        var deviceTablet = 2;
        var deviceMobile = 3;

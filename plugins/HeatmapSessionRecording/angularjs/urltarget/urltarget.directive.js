...
(function () {
    ...
    piwikHsrUrlTarget.$inject = ['piwik', 'piwikApi', '$filter'];

    function piwikHsrUrlTarget(piwik, piwikApi, $filter){

        var translate = $filter('translate');
        var targetPromise = piwikApi.fetch({method: 'HeatmapSessionRecording.getAvailableTargetPageRules'});
                ...
                targetPromise.then(function (targetAttributes) {
                    ...
                    angular.forEach(targetAttributes, function (targetAttribute) {
                        ...
                        if (scope.allowAny && targetAttribute.value == 'url') {
                            scope.targetOptions[targetAttribute.value].push({value: translate('HeatmapSessionRecording_TargetTypeIsAny'), key: 'any'});
                        }
                        ...
                        angular.forEach(targetAttribute.types, function (type) {
                            ...
                            scope.targetOptions[targetAttribute.value].push({value: translate('HeatmapSessionRecording_TargetTypeIsNot', type.name), key: 'not_' + type.value});
                        });
                    });

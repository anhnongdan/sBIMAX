...
(function () {
    ...
    SessionRecordingVisController.$inject = ['$scope', 'piwikApi', '$timeout', '$interval', '$filter', '$sce', '$q'];
    ...
    function SessionRecordingVisController($scope, piwikApi, $timeout, $interval, $filter, $sce, $q) {
        ...
        $timeout(function () {
            $timeout(function () {
                ...
                iframeLoadedPromise.then(function () {
                    ...
                    if (!recordingIframe.isSupportedBrowser()) {
                        return;
                    }
                    ...
                    function replayEvent(event)
                    {
                        ...
                        if (event.event_type == EVENT_TYPE_MOVEMENT) {
                            ...
                            if (event.selector) {
                                recordingIframe.findElement(event.selector).val(event.text).change();
                            }
